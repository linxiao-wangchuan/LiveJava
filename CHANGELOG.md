...
**新增**: 编辑器背景图系统 — 上传/粘贴/切换/删除，存磁盘 `main/backgrounds/`，缩略图网格管理，「无」选项清空，上传限制 50MB。
**新增**: 编辑器背景视频系统 — 📷背景图/🎬背景频模式切换互斥，视频存 `main/background_videos/` 独立目录，支持覆盖/完整填充，音量控制（默认静音），上传限制 150MB。
